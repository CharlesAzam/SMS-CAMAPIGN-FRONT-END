...
export class BannerEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      if (params.id !== "new") {
        this.bannerService.findById(params.id).subscribe(
          (response: any) => {
            if (response.status === 200) {
              ...
              this.bannerForm.setValue({
                ...
                description: this.bannerModel.description
                  ? this.bannerModel.description
                  : "",
                status: String(this.bannerModel.status)
                  ? String(this.bannerModel.status)
                  : "",
                image: this.bannerModel.image ? this.bannerModel.image : "",
                content: this.bannerModel.content
                  ? this.bannerModel.content
                  : "",
                subtitle: this.bannerModel.subtitle
                  ? this.bannerModel.subtitle
                  : "",
                priority: this.bannerModel.priority
                  ? this.bannerModel.priority
                  : "",
                URL: this.bannerModel.URL ? this.bannerModel.URL : "",
                categories: this.bannerModel.categories._id
                  ? this.bannerModel.categories._id
                  : ""
  ...
  save() {
    if (this.imageUrl) this.bannerForm.value["image"] = this.imageUrl;
    if (this.bannerModel) {
      Object.assign(this.bannerModel, this.bannerForm.value);
      this.bannerService.update(this.bannerModel).subscribe(
        (response: any) => {
